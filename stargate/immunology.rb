module Stargate
  module Immunology
    ...
    def self.handle_telemetry(subsystem, severity, message)
    ...
    def self.process_threat(level, evidence)
      action = {
        warning:     :observe,
        recoverable: :recall,
        critical:    :freeze_and_recall,
        paradox:     :absolute_stasis
      }[level]
    ...
    def self.execute_action(action, level, evidence)
      case action
      when :observe
